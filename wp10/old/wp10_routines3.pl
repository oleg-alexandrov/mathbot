...
@INC = ('/m1/aoleg/perl', '/m1/aoleg/perl/lib/perl5/5.8.3/', '/m1/aoleg/perl/lib/perl5/site_perl/5.8.3/', '/m1/aoleg/perl/', '/m1/aoleg/wp/utils', @INC);
#use lib '/m1/aoleg/perl'; # path to perl modules
use lib $ENV{HOME} . '/public_html/cgi-bin/wp/modules'; # path to perl modules
require 'bin/wikipedia_fetch_submit3.pl'; 
require 'bin/wikipedia_login3.pl';
...
#  exit(0);
  ...
  # global log, try to submit only a few times, this may fail as it is a large file
  my $little = 20; &wikipedia_submit($Todays_logname, "All for $date.", $todays_logs, $little, $Sleep_submit);
...
sub update_index{
  ...
  $text = &wikipedia_fetch($Index_file, $Attempts, $Sleep_fetch);
  if ($text =~ /^(.*?$Bot_tag.*?\n)(.*?)($Bot_tag.*?)$/s){
  ...
  &wikipedia_submit($Index_file, "Update index", $text, $Attempts, $Sleep_submit);
}
...
sub fetch_list_subpages{
  ...
  $text = &wikipedia_fetch($file, $Attempts, $Sleep_fetch);
  ...
  foreach $subpage (@subpages){
    ...
    $subpage_text = &wikipedia_fetch($subpage, $Attempts, $Sleep_fetch);
    $text = $text . "\n" . $subpage_text;
...
sub extra_categorizations {
  ...
  foreach $subject_category (@subjects) {
	...
	next;
      ...
      &wikipedia_submit($file, $edit_summary, $text, $Attempts, $Sleep_submit);
    }
  }
...
sub get_wikiproject {
  ...
  $text=&wikipedia_fetch($wikiproject . ".wiki", $Attempts, $Sleep_fetch);

  # if the wikiproject was not guessed right, maybe the plural is wrong (frequent occurence)
  if ($text =~ /^\s*$/){
    $wikiproject_alt = $wikiproject . "s";
    $text=&wikipedia_fetch($wikiproject_alt . ".wiki", $Attempts, $Sleep_fetch);
