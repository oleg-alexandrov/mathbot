...
sub get_server_page {
    ...
    # The oleg hack, convert to unicode
    #print "$doc\n";
    #exit(0);
    #$doc =~ s/\&amp;/\%26/g;
    ...
    # another Oleg hack
    print "****************headline before $headline\n";
    $headline =~ s/\&/\%26/g;
    print "****************headline after $headline\n";
...
# called from wikipedia_submit.
# It differs from _upload_file in the headline encoding Oleg hack below

sub _upload_file3 {
  my ($self, $filename, $text) = @_;

  my $commit = 1; # don't preview, but actually submit

  WWW::Mediawiki::Client::URLConstructionException->throw("No server URL specified.")
     unless $self->{host};
  
  WWW::Mediawiki::Client::URLConstructionException->throw("No commit message specified.")
     unless $self->{commit_message};

  WWW::Mediawiki::Client::CommitException->throw("Could not determine charset for uploading to this server.")
     unless $self->encoding;
  ...
  # note below that $headline is encoded to utf8, that's different
  # from the _upload_file routine above
  $headline = Encode::encode('utf8', $headline);
